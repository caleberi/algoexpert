	...
	if fastest:
		...
		if fastestRedRider > fastestBlueRider:
			for idx in range(length):
				maximumTotalSpeed+=max(redShirtSpeeds[length-1-idx],blueShirtSpeeds[idx])
			return maximumTotalSpeed
		else:
			for idx in range(length):
				maximumTotalSpeed+=max(blueShirtSpeeds[length-1-idx],redShirtSpeeds[idx])
			return maximumTotalSpeed
	else:
		minimumTotalSpeed=0
		length=len(redShirtSpeeds)
		for idx in range(length):
			minimumTotalSpeed+=max(redShirtSpeeds[length-1-idx],blueShirtSpeeds[length-1-idx])
